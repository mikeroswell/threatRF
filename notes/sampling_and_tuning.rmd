...
fit_rf <- function(data, formu, sampling = NULL, tuneMethod = "none"){ # set up to take formula as a string
  ...
  fC = caret::trainControl(method = tuneMethod
      ...
      , repeats = ifelse(tuneMethod != "none", 10, NA)
      , sampling =  sampling
      , classProbs = TRUE
...
outer_folds <- createMultiFolds(example_train_dat, k = 10, times = 1)
...
pred_list <- future_map2(function(x, y) { # compute predictions on each of the 100 test sets
   pre = predict(fold_fits[[x]]
                , test_dat[-y, c(1, 3:8, 10:21
                         , 23:26, 28:length(names(example_train_dat)))]
                , type = "prob")
  predictions = prediction(pre[,2], test_dat[-y, 2])
  ...
  data.frame(pre
             , truth = test_dat[-y, 2]
             , accuracy = preval ==test_dat[-y, 2]
             , oob= fold_fits[[x]]$finalModel$err.rate[,1])
             , positive=fold_fits[[x]]$finalModel$err.rate[,2])
             , negative = x$finalModel$err.rate[,3])
             , predictions = predictions@predictions 
             , auc
             , mod = x
  ...
  predict(x, example_train_dat[-y,])}, 
  x = 1:length(fold_fits), y = folds)
