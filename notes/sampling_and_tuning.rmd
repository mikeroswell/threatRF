...
all_mods<-map(mlist
  ...
  rf = fit_rf(formu = my_mod
          ...
          , if(is.null(mopt[[1]][2])){sampling = NULL}else{
...
map(all_mods, function(rf){
    rf$results %>% ggplot(aes(x=mtry, y=Kappa)) +
    labs(x="Number of variables\nsampled at each split" , y="Cohen's kappa", main="Random forest model tune") +
    geom_line() +
    geom_point() +
    theme_bw()})
...
default of $\sqrt(N_features)$ seems like it's not that far from optimal here,
...
preds<-map_dfr(1:6, function(x){
  pre = predict(all_mods[[x]]
                , dat[,c(1, 3:8, 10:21
                         , 23:26, 28:length(names(example_train_dat)))])
  truth = dat[,2]
  data.frame(pre, truth, mod = mlist[x])
})
  
preds %>% 
  mutate(correct = pre ==truth) %>% 
  group_by(mod, truth) %>% 
  summarize(rate = round(sum(correct)/n(),2)
            , instances = n())
```
